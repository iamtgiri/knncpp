...
/*
==================================================================
Instead of using cxxopts, I have built a simple custom CLI parser for better control and fewer dependencies.
This keeps the project lightweight and focused on core functionality.
==================================================================
*/
...
/*
==================================================================
Replaced the cxxopts-based CLI parser with a custom implementation for simplicity and fewer dependencies.
==================================================================
*/
...
int main(int argc, char *argv[])
{
    ...
    load_csv(opt.data, features, labels);
    std::cout << "[INFO] Loaded data with " << features.size() << " samples.\n";
    ...
    if (opt.mode == "brute")
    {
        KNN knn(opt.k_neighbors);
        knn.fit(X_train, y_train);
        y_pred = knn.predict(X_test, opt.parallel);
    }
    else if (opt.mode == "kdtree")
    {
        KDTreeKNN knn(opt.k_neighbors);
        knn.fit(X_train, y_train);
        y_pred = knn.predict(X_test, opt.parallel);
    }
    else
    {
        std::cerr << "[ERROR] Invalid mode. Use 'brute' or 'kdtree'.\n";
        return 1;
    }
